...
(function() {
  ...
  if (String(typeof window) !== "undefined") {
    ["webkit", "moz"].forEach(function(key) {
      ...
      _cancelAnimationFrame =
        ...
        window[key + "CancelAnimationFrame"] ||
        null;
    });
  }
  ...
  function Gamepad() {
    ...
    this._handleKeyboardEventListener = this._handleKeyboardEventListener.bind(
      this
    );
  ...
  Gamepad.prototype._handleGamepadAxisEventListener = function(controller) {
    ...
    if (controller && controller.connected) {
      Object.keys(self._keyMapping.axes).forEach(function(key) {
        var axes = Array.prototype.slice.apply(
          controller.axes,
          self._keyMapping.axes[key]
        );

        if (
          Math.abs(axes[0]) > self._threshold ||
          Math.abs(axes[1]) > self._threshold
        ) {
  ...
  Gamepad.prototype.on = function(type, button, callback, options) {
    ...
    if (
      Object.keys(this._handlers.gamepad).indexOf(type) !== -1 &&
  ...
  Gamepad.prototype.setCustomMapping = function(device, config) {
    if (this._keyMapping[device] !== undefined) {
      ...
      throw new Error(
        'The device "' + device + '" is not supported through gamepad.js'
      );
    }
  };
